...
marker_idx = 1:120:1000;
subplot(1,2,1)
plot(slip,'b-')

title('')
grid on;
axis([0 24 0 1]);%限制轴的显示宽度 前面是x轴，后面是y 轴
%%%%%%%%%%%基本设置%%%%%%%%%%%%%%
...
axis([0 24 0 30]);%限制轴的显示宽度 前面是x轴，后面是y 轴
grid on;
...
set(gca,'FontSize',10); %legend、坐标轴数字
set(findobj(get(gca,'Children'),'LineWidth',0.5),'LineWidth',2);  %设置线宽
set(gcf,'Position',[500 350 720 480]); %前两个是初始位置坐标，后两个是初始分辨率

%%%%%%%%%%%基本设置%%%%%%%%%

% 绘制总标题
suptitle('Braking simulation under low adhesion condition')
